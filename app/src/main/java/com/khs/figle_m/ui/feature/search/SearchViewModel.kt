            ...
            .collectLatest { result ->
                when (result) {
                    ...
                    is CommonResult.Fail.Error -> {
                        _uiState.value = SearchUIState.Failed(errorCode = result.resultCode, errorMsg = result.errorMsg)
                    }

                    is CommonResult.Fail.Exception -> {
                        _uiState.value = SearchUIState.Failed(errorCode = -1, errorMsg = result.exception.message)
                    }
                    else -> {
